...
class PPYOLOERandomDistort(object):
...
class PPYOLOEPad(object):
    ...
    def __call__(self, results):
        ...
        if self.pad_mode == 0:
            return results
        gt_bboxes = self.apply_bbox(gt_bboxes, offsets)
        results['gt_bboxes'] = gt_bboxes
        return results


@PIPELINES.register_module()
class PPYOLOERandomExpand(object):
...
@PIPELINES.register_module()
class PPYOLOERandomCrop(object):
    ...
    def _get_crop_size(self, img_shape, gt_bbox):
        h, w = img_shape[:2]
        ...
        for thresh in thresholds:
            ...
            for i in range(self.num_attempts):
                ...
                iou = self._iou_matrix(gt_bbox, np.array(
                ...
                if valid_ids.size > 0:
                    ...
                    break
            if found:
                return found, crop_box, cropped_box, valid_ids
        return False, 0, 0, 0

    def __call__(self, results):
        img = results['img'].copy()
        gt_bboxes = results['gt_bboxes'].copy()
        gt_labels = results['gt_labels'].copy()
        # gt_labels_ignore = results['gt_labels_ignore'].copy()
        found, crop_box, cropped_box, valid_ids = self._get_crop_size(
            img.shape[:2], gt_bboxes)

        if found:
            img = self._crop_image(img, crop_box)
            gt_bboxes = np.take(cropped_box, valid_ids, axis=0)
            gt_labels = np.take(gt_labels, valid_ids, axis=0)
            # gt_labels_ignore = np.take(gt_labels_ignore, valid_ids, axis=0)
